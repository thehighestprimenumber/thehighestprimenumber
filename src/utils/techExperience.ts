...
export function calculateTechExperience(): Map<string, TechExperience> {
  ...
  resume.experience.forEach(exp => {
    ...
    exp.technologies.forEach(tech => {
      ...
      const currentFirst = parseDate(techExp.firstUsed.split(' ')[1] + '-' + getMonthNumber(techExp.firstUsed.split(' ')[0]))
      const currentLast = techExp.lastUsed === 'Present' ? new Date() : parseDate(techExp.lastUsed.split(' ')[1] + '-' + getMonthNumber(techExp.lastUsed.split(' ')[0]))
